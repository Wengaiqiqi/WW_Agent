...
def _init_qps_from_env():
    ...
    if qps_env:
        try:
            qps = float(qps_env)
            _min_request_interval = 1.0 / qps if qps > 0 else 0
        except ValueError:
            pass  # 无效值，使用默认值
...
def set_qps(qps):
    ...
    """
    global _min_request_interval
    _min_request_interval = 0 if qps <= 0 else 1.0 / qps
    ...
    while True:
        ...
        try:
            if sys.platform == "win32":
                ...
                fd = open(lock_file, "a+")  # noqa: SIM115
                try:
        ...
        while True:
            ...
            try:
                if self._platform == "win32":
                    ...
                    fd = open(self.lock_file, "a+")  # noqa: SIM115
                    try:
                        msvcrt.locking(fd.fileno(), msvcrt.LK_NBLCK, 1)
                        self.lock_fd = fd
                        return True
